...
import { fetchUserServers } from '../../store/servers';

export default function SideNavBar() {
	const sessionUser = useSelector((state) => state.session.currentUser);
	const dispatch = useDispatch();
	const userServers = useSelector((state) =>
	    Object.keys(state.servers).length === 0  ? [] : Object.values(state.servers)
	);

	useEffect(() => {
		dispatch(fetchUserServers(sessionUser.id));
	}, [userServers.length]);
