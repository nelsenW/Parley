	...
	return (
		<div className='friend-form-modal'>
			...
			<div className='known-users'>
				...
				{filteredUsers
					?.map((user) => <FriendCard user={user} handleSelect = {handleSelect} setFriendId ={setFriendId}/>)
					.slice(0, 3)}
			</div>
			{submit && (
				<button type='submit' onClick={handleSubmit} className='add-friend-button'>
					Add Friend!
				</button>
			)}
		</div>
		...
		<button
			...
			{user.photo ? (
				...
				<i
					...
					style={{ backgroundColor: `${user.color}` }}></i>
			)}
			<h3>{user.username}</h3>
		</button>
	);
